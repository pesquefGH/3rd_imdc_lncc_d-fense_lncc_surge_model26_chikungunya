...
|_>  LNCC_SurgeModel26_CG  (model source and results)
...
alignDC_CG.m  % function that aligns the smoothed incidence (surge) curves (52 epiweeks) so that their maxima are aligned to the middle of the window (sample 26).
...
[DCalign, ind_max]=alignDC_CG(DC,th); % aligns surges (peak above th) so that they all peak
...
P_est = lsqcurvefit(fun,P,n,typ_DC,[100;0.1;24],[370000;0.5;28],options); % find estimated model parameters P_est
